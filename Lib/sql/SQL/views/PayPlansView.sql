...
SELECT 
	...
	(select `Past Due` -- I already calculated this in the scheduleOnPlanView so I just went through that
	from `vwscheduleonplan` 
	where  `vwscheduleonplan`.`Pay Plan ID` = `payplans`.`PayPlanID`
		and `PPF Mark`  = 'Present') as 'Past Due',
	(select `Past Due CSS ID` -- I already calculated this in the scheduleOnPlanView so I just went through that
	from `vwscheduleonplan` 
	where  `vwscheduleonplan`.`Pay Plan ID` = `payplans`.`PayPlanID`
		and `PPF Mark`  = 'Present') as 'Past Due CSS ID',
